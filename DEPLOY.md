...
workflow to complete (check https://github.com/five30am/cartdex/actions).

```bash
docker pull ghcr.io/five30am/cartdex:latest
```
...
```bash
scp ~/projects/cartdex/docker-compose.yml root@10.10.5.252:/opt/cartdex/docker-compose.yml
```
...
```bash
ssh root@10.10.5.252 "cd /opt/cartdex && docker compose up -d --force-recreate"
```
...
```bash
ssh root@10.10.5.252 "docker logs cartdex --tail 30"
```
...
```bash
ssh root@10.10.5.252 "docker exec cartdex touch /roms/.trash/.write-test && echo 'rw confirmed' && docker exec cartdex rm /roms/.trash/.write-test"
```
...
```bash
# Copy script to host
scp ~/projects/cartdex/scripts/purge-trash.sh root@10.10.5.252:/opt/cartdex/scripts/purge-trash.sh
ssh root@10.10.5.252 "chmod +x /opt/cartdex/scripts/purge-trash.sh"
...
Test run (dry mode):
```bash
ssh root@10.10.5.252 "DRY_RUN=1 /opt/cartdex/scripts/purge-trash.sh"
```
...
Open CartDex in the browser and verify:
...
ssh root@10.10.5.252 "
  sed -i 's|/mnt/roms:/roms:rw|/mnt/roms:/roms:ro|' /opt/cartdex/docker-compose.yml
  cd /opt/cartdex && docker compose up -d --force-recreate
"
```
...
- The purge script operates on `/mnt/roms/.trash/` directly (host path). If the
  ...
  set it as an env var: `TRASH_ROOT=/new/path /opt/cartdex/scripts/purge-trash.sh`
- DB path on host: `/var/lib/docker/volumes/cartdex_data/_data/cartdex.db`
