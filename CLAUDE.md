# CartDex -- Project Context for Claude

Self-hosted ROM library. Next.js 16 App Router + SQLite (better-sqlite3 + Drizzle). Repo: `five30am/cartdex`.

## Where things run (IMPORTANT -- don't guess)

- **Container host:** `10.10.5.252` (Portainer) -- NOT the Claude VM you're running on
- **Container name:** `cartdex`
...
- **Volumes:** `cartdex_data` (DB + migrations) * `cartdex_artwork` (box art) * bind `/mnt/roms:/roms:rw` (Unraid CIFS)
...
```bash
ssh root@10.10.5.252 "docker exec cartdex node -e 'const db=require(\"better-sqlite3\")(\"/data/cartdex.db\"); console.log(JSON.stringify(db.prepare(\"<SQL>\").all()));'"
```

Or delegate the query to Holt and let him run it. DB path inside container: `/data/cartdex.db`.
...
API mutations require `CARTDEX_API_TOKEN` in the container environment.
Set `X-Api-Token: <value>` header on all POST/PATCH/PUT/DELETE routes.
If not set, all mutations are locked (fail-closed).
...
1. Push to main
2. Self-hosted GH Actions runner (on the Claude VM) builds the image and pushes to `ghcr.io/five30am/cartdex:latest`
...
Pre-GitOps backup at `/opt/romvault.pre-gitops-2026-04-14` on 10.10.5.252 -- delete after 2026-04-21 if stable (was stable before rename, keep for rollback context).
...
- Repo was `five30am/romvault` before 2026-05-21; renamed to `five30am/cartdex` (hard cutover -- RomVault name conflicts with established product since early 2000s).
- Steam Deck companion app (no code yet, planning complete) renamed from RomCourier to CartCrate.
- Local directory was `~/projects/romvault/`, now `~/projects/cartdex/`.
